...
# This is a test suite for the fluent-plugin-out_rawtcp settings.
...
os::test::junit::declare_suite_start "Remote Rawtcp Configuration Tests"
...
os::log::info Starting fluentd-plugin-out-rawtcp tests at $( date )
os::log::info Edit fluentd daemonset configmap

oc get configmap/logging-fluentd -o yaml | sed '/## matches/ a\
      <match **>\
        @type copy\
        @include configs.d/user/output-extra-raw_tcp.conf\
      </match>' | oc replace -f -
        oc patch configmap/logging-fluentd --type=json --patch '[{ "op": "add", "path": "/data/output-extra-raw_tcp.conf", "#": "generated config file output-extra-raw_tcp.conf" }]' 2>&1
        oc patch configmap/logging-fluentd --type=json --patch '[{ "op": "replace", "path": "/data/output-extra-raw_tcp.conf", "value": "\
  <store>\n\
   @type rawtcp\n\
   flush_interval 1\n\
    <server>\n\
...
if [ -n "${my_remote_syslog_host:-}" ] ; then
  ...
  os::cmd::try_until_success "oc exec $fpod find /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
  os::cmd::expect_success_and_text "oc exec $fpod grep 'remote_syslog' /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf" "remote_syslog ${my_remote_syslog_host}"
fi
...
fpod=$( get_running_pod fluentd )
os::cmd::try_until_failure "oc exec $fpod find /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
...
fpod=$( get_running_pod fluentd )
os::cmd::try_until_text "oc exec $fpod grep '<store>' /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf | wc -l" '^2$'


os::log::info Test 4, making sure tag_key=message does not cause out-rawtcp plugin crash
...
fpod=$( get_running_pod fluentd )
os::cmd::try_until_success "oc exec $fpod find /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
os::cmd::expect_success "oc exec $fpod grep 'tag_key message' /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
os::cmd::expect_success_and_not_text "oc logs $fpod" "nil:NilClass"

extra_rsyslog_artifacts=$ARTIFACT_DIR/rsyslog-artifacts.txt
if [ -n "${DEBUG:-}" ] ; then
  echo Test 4, making sure tag_key=message does not cause out-rawtcp plugin crash > $extra_rsyslog_artifacts
  ...
  echo "output-out-rawtcp.conf: " >> $extra_rsyslog_artifacts

  oc exec $fpod -- cat /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf >> $extra_rsyslog_artifacts
fi

os::log::info Test 5, making sure tag_key=bogus does not cause out-rawtcp plugin crash
...
fpod=$( get_running_pod fluentd )
os::cmd::try_until_success "oc exec $fpod find /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
os::cmd::expect_success "oc exec $fpod grep 'tag_key bogus' /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
os::cmd::expect_success_and_not_text "oc logs $fpod" "nil:NilClass"

if [ -n "${DEBUG:-}" ] ; then
  echo Test Test 5, making sure tag_key=bogus does not cause out-rawtcp plugin crash >> $extra_rsyslog_artifacts
...
if [ "$savemuxdc" != "" ]; then
  ...
  if [ -n "${my_remote_syslog_host:-}" ] ; then
    ...
    if [ -n "${DEBUG:-}" ] ; then
      ...
      echo "output-out-rawtcp.conf: " >> $extra_rsyslog_artifacts
      oc exec $mpod -- cat /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf >> $extra_rsyslog_artifacts
    fi
    os::cmd::try_until_success "oc exec $mpod find /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
    os::cmd::expect_success_and_text "oc exec $mpod grep 'remote_syslog' /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf" "remote_syslog ${my_remote_syslog_host}"
    os::log::debug "$( oc scale --replicas=0 dc logging-mux )"
    os::cmd::try_until_text "oc get dc logging-mux -o jsonpath='{ .status.replicas }'" "0" $FLUENTD_WAIT_TIME
  fi
  ...
  mpod=$( get_running_pod mux )
  os::cmd::try_until_success "oc exec $mpod find /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf"
  os::cmd::expect_success_and_text "oc exec $mpod grep 'remote_syslog' /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf" "remote_syslog 127.0.0.1"
  if [ -n "${DEBUG:-}" ] ; then
    oc logs $mpod >> $extra_rsyslog_artifacts 2>&1
    echo "output-out-rawtcp.conf: " >> $extra_rsyslog_artifacts
    oc exec $mpod -- cat /etc/fluent/configs.d/dynamic/output-out-rawtcp.conf >> $extra_rsyslog_artifacts
