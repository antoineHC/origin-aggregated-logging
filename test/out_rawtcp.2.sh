...
create_forwarding_fluentd() {
  ...
  while [ $id -lt $cnt ]; do
    ...
    if [ $id -eq 0 ]; then
      oc get daemonset/logging-fluentd -o yaml | \
        sed -e "s/logging-infra-fluentd:/logging-infra-forward-fluentd0:/" \
            -e "s/name: logging-fluentd/name: logging-forward-logstash0/" \
      ...
      oc get daemonset/logging-fluentd -o yaml | \
        sed -e "s/logging-infra-fluentd:/logging-infra-forward-fluentd1:/" \
            -e "s/name: logging-fluentd/name: logging-forward-logstash1/" \
    ...
    oc set volumes daemonset/logging-forward-logstash${id} --add --overwrite \
    ...
    os::cmd::try_until_text "oc get pods -l component=forward-fluentd${id}" "^logging-forward-logstash${id}-.* Running "
    POD=$( oc get pods -l component=forward-fluentd${id} -o name )
